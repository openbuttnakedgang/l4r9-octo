...
impl OSpi {
    ...
    pub fn init(&mut self) {
        ...
        rcc.ahb2enr.write(|w| w.ospimen().set_bit());

        // rcc.ahb2rstr.write(|w| w.ospimrst().set_bit());
        // rcc.ahb3rstr.write(|w| w.ospi2rst().set_bit());
        ...
        let _clk = gpioi.pi6.into_af5(&mut gpioi.moder, &mut gpioi.afrl).set_speed(Speed::High);
        let _ncs = gpiog.pg12.into_af5(&mut gpiog.moder, &mut gpiog.afrh).set_speed(Speed::High);
        let _dqs = gpiog.pg15.into_af5(&mut gpiog.moder, &mut gpiog.afrh).set_speed(Speed::High);

        let _io0 = gpioi.pi11.into_af5(&mut gpioi.moder, &mut gpioi.afrh).set_speed(Speed::High);
        let _io1 = gpioi.pi10.into_af5(&mut gpioi.moder, &mut gpioi.afrh).set_speed(Speed::High);
        let _io2 = gpioi.pi9.into_af5(&mut gpioi.moder, &mut gpioi.afrh).set_speed(Speed::High);
        let _io3 = gpioh.ph8.into_af5(&mut gpioh.moder, &mut gpioh.afrh).set_speed(Speed::High);
        let _io4 = gpioh.ph9.into_af5(&mut gpioh.moder, &mut gpioh.afrh).set_speed(Speed::High);
        let _io5 = gpioh.ph10.into_af5(&mut gpioh.moder, &mut gpioh.afrh).set_speed(Speed::High);
        let _io6 = gpiog.pg9.into_af5(&mut gpiog.moder, &mut gpiog.afrh).set_speed(Speed::High);
        let _io7 = gpiog.pg10.into_af5(&mut gpiog.moder, &mut gpiog.afrh).set_speed(Speed::High);
        ...
        ospi.tcr.modify(|_, w| unsafe { w.dcyc().bits(0) });
        while ospi.sr.read().busy().bit_is_set() {}

        // Заполнение instruction register кодом команды
        ospi.ir.modify(|_, w| unsafe { w.instruction().bits(0x06) });
        while ospi.sr.read().busy().bit_is_set() {}
        ...
        // Заполнение data length register
        ospi.dlr.write(|w| unsafe { w.dl().bits(1u32 - 1) });
        while ospi.sr.read().busy().bit_is_set() {}
        ...
        // Заполнение instruction register кодом команды
        ospi.ir.modify(|_, w| unsafe { w.instruction().bits(0x05) });
        while ospi.sr.read().busy().bit_is_set() {}

        // Указание режима работы octospi, обязательно последним
        ...
        // // Заполнение instruction register кодом команды
        // ospi.ir.modify(|_, w| unsafe { w.instruction().bits(0xEC13) });
        // while ospi.sr.read().busy().bit_is_set() {}
        ...
        //             unsafe {
        //                 info!("3");
                        buffer[0] = unsafe {ptr::read_volatile(&ospi.dr as *const _ as *const u8)};
                        buffer[1] = unsafe {ptr::read_volatile(&ospi.dr as *const _ as *const u8)};
                        buffer[2] = unsafe {ptr::read_volatile(&ospi.dr as *const _ as *const u8)};
        //             }
        //         } else {
        //             println!("Error");
        //             // OVERFLOW
        ...
        println!("Finally!! Congratulations: {:x?}", &buffer[..]);
